...
class Lobby extends EventEmitter
{
	...
	promoteAllPeers()
	{
		...
		setTimeout(function()
		{
			this._promoteAllPeersLoop(this, peerArray, 0);
		}, 200);
	}
	...
	_promoteAllPeersLoop(lobby, peerArray, index)
	{
		...
		const peer = this._peers[peerArray[index]];

		if (peer && this._peers[peer.id] && !this._peers[peer.id].closed)
		...
		setTimeout(function()
		{
			this._promoteAllPeersLoop(lobby, peerArray, index);
		}, 200);
	}
