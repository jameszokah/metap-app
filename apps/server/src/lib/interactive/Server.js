// write es6 import for the above modules
...
// Unix socket path.
const SOCKET_PATH_UNIX = '/tmp/metap-server.sock';
const SOCKET_PATH_WIN = path.join('\\\\?\\pipe', process.cwd(), 'metap-server');
...
export default async function(rooms, peers) {
    try {
        ...
        await new Promise((resolve) => {
            try { fs.unlinkSync(SOCKET_PATH); } catch (error) {
        ...
        // eslint-disable-next-line no-console
        console.error('interactiveServer error:', error);
    }
};